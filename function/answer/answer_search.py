...
class answerearcher:
    def __init__(self):
        # 设定需要访问的地址，用户名和密码，以连接数据库
        self.g = Graph("http://localhost:7474/", auth = ("neo4j", "Zty686240722"))   # 修改对应的neo4j用户名和密码
    ...
    def search_in_database(self,sqls):
        ...
        for sql_ in sqls:
            ...
            for query in queries:
                # ress例如[{'n.name': '篮球', 'm.name': '张三'}, {'n.name': '篮球', 'm.name': '张三'}, {'n.name': '篮球', 'm.name': '张三'}, {'n.name': '篮球', 'm.name': '张三'}, {'n.name': '篮球', 'm.name': '张三'}]
                ress = self.g.run(query).data()
                print(5555555555555555555555555555555555555555555555, ress)
                answer+= ress
